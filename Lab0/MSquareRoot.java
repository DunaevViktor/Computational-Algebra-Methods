...
public class MSquareRoot {
    ...
    public MSquareRoot(double[][] sharedMatrix) {
        rows = (byte)sharedMatrix.length;
        cols = (byte)sharedMatrix.length;
        determinant = 1;
        matrix = new double[rows][cols];
    ...
    public void outputResidualVector() throws ArrayIndexOutOfBoundsException {
        ...
        System.out.println("Норма вектора невязки равна: " + Math.abs(res[index]));
    }

    public void outputDeterminant() throws ArrayIndexOutOfBoundsException {
        System.out.printf("Определитель матрицы = %7.4f%n", getDeterminant());
    }

    public void findSymmetric() throws ArrayIndexOutOfBoundsException {
        double[][] transposedMatrix = MyMatrix.transposeMatrix(matrix);
        symmetricMatrix = MyMatrix.matrixCompositionMatrix(transposedMatrix, matrix);
